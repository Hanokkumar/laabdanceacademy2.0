...
import {
  Plus, Edit2, Trash2, LogOut, Calendar, MapPin, DollarSign,
  LayoutDashboard, Loader2, Search, Eye
} from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-white font-manrope">Admin Dashboard</h1>
                <p className="text-gray-400 font-dm-sans text-xs">Welcome, {user?.username}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <button
                onClick={() => navigate('/')}
                className="text-gray-400 hover:text-white font-dm-sans text-sm transition-colors flex items-center gap-1.5"
              >
                <Eye size={16} />
                View Site
              </button>
              <button
                ...
                className="bg-white/10 hover:bg-white/20 text-white font-dm-sans text-sm px-4 py-2 rounded-lg transition-all flex items-center gap-1.5"
              >
                <LogOut size={16} />
                Logout
              </button>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredEvents.map((event) => (
              <div key={event.id} className="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md transition-shadow group">
                {/* Image */}
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setDeleteConfirm(null)}>
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-gray-500 font-dm-sans text-sm mt-2">This action cannot be undone. The event and its image will be permanently removed.</p>
